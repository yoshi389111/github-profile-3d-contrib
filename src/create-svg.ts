...
export const createSvg = (userInfo: type.UserInfo): string => {
    ...
    radar.createRadarContrib(svg, userInfo, radarX, 70, radarWidth, radarHeight);
    ...
    group.append("text")
        ...
        .attr('text-anchor', 'start')
        .text(userInfo.totalStargazerCount < 1000 ? userInfo.totalStargazerCount : "999+")
        .attr('fill', fgcolor);
    ...
    const period = `${startDate.toISOString().substring(0, 10)}/${endDate.toISOString().substring(0, 10)}`;
